...
const NAV_ITEMS: NavItem[] = [
  ...
  },
  {
    label: "Mes favoris",
    to: "/favorites",
    icon: <Icon d="M12 3.5l2.78 5.63 6.22.9-4.5 4.39 1.06 6.2L12 17.7l-5.56 2.92 1.06-6.2L3 10.03l6.22-.9L12 3.5z" />,
  },
];

function linkClass({ isActive }: { isActive: boolean }) {
  return `flex items-center gap-3 rounded-md px-3 py-2 text-sm transition ${
    isActive
...
export default function Sidebar({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <>
      {/* Fond assombri derrière le tiroir mobile, ferme au clic en dehors */}
      {open && (
        <div
      ...
      >
        ...
        <nav className="flex-1 space-y-1 px-3">
          ...
          {/* Modération/Administration : réservées admin/modérateur, inutile de montrer un lien
              vers une page à laquelle l'utilisateur n'a pas accès. */}
          {isModerator && (
            <NavLink to="/moderation" onClick={onClose} className={linkClass}>
