...
describe("requireRole", () => {
  it("laisse passer un utilisateur avec le bon rôle", async () => {
    const request = makeRequest();
    request.user = { ...validPayload, role: "admin" };
    const reply = makeReply();
    await requireRole("admin", "moderator")(request, reply);
    expect(reply.status).not.toHaveBeenCalled();
  });

  it("rejette un utilisateur avec un rôle insuffisant", async () => {
    const request = makeRequest();
    request.user = { ...validPayload, role: "student" };
    const reply = makeReply();
    await requireRole("admin", "moderator")(request, reply);
    expect(reply.status).toHaveBeenCalledWith(403);
    expect(reply.send).toHaveBeenCalledWith({ error: "Accès réservé" });
  });

  it("rejette une requête sans request.user (non authentifiée)", async () => {
    const request = makeRequest();
    const reply = makeReply();
